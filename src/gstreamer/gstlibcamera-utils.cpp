					 ...
					 GstCaps *caps)
{
	...
	for (i = 0; i < gst_caps_get_size(ncaps); i++) {
		...
		g_autofree const gchar *colorimetry = gst_structure_get_string(ns, "colorimetry");
		gst_video_colorimetry_from_string(&colorimetry_old, colorimetry);

		/* Validate the configuration and check if new colorimetry
		 * can be applied to the sensor.
		 */
